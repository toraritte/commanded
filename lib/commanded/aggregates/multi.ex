defmodule Commanded.Aggregate.Multi do
  ...
  # 2019-02-25_0901 NOTE `apply_events/2` vs Aggregate.Multi
  @doc """
  + `apply_events/2`

    Apply all resulting events (generated by executing a
    command) sequentially using `Enum.reduce/3`.

  + `Aggregate.Multi`

    Allows registering functions,  that will be executed
    in  sequence   (using  `Enum.reduce/3`),   and  each
    iteration  will   apply  generated  events   to  the
    aggregate state, which in turn  will be fed into the
    next iteration.  Therefore each  registered function
    will receivethe most up-to-date aggregate struct.

  In   the  moduledoc,   the   events  generated   are
  `MoneyWithdrawn` and  `AccountOverdrawn`. The former
  is  a domain  event,  that could  be  used by  other
  contexts,  whereas the  latter is  the state  of the
  `BankAccount` aggregate. This could also be an event
  (e.g., send notification to the user), but it mainly
  describes what state the aggregate is in.

  Another reason why it would be cleaner using a state
  machine, because then the distinction between domain
  events, and state  transition responding to internal
  events, would be explicit.
  """
